...
export default function CoreBenefits() {
  return (
    <section className="w-full bg-gradient-to-b from-[#F4D45A] to-[#F4C644] min-h-screen flex flex-col items-center justify-center px-4 py-12 md:py-20">
      {/* Header */}
      <div className="text-center mb-12 md:mb-16 max-w-4xl">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-[#3D1F3F] mb-4">
          The Value Behind the <span className="text-blue-600">Platform</span>
        </h1>

        <p className="text-gray-700 text-base md:text-lg leading-relaxed">
          Easily track and manage your sales pipeline with real-time updates, visual deal stages to keep your sales flow
          seamless.
        </p>
      </div>

      {/* Cards Grid with Center Circle */}
      <div className="relative w-full max-w-6xl mx-auto">
        {/* Grid Layout */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 lg:gap-16 px-6 md:px-8 py-12 md:py-16">
          {/* Top Left Card */}
...
function Card({ card }: { card: (typeof cards)[0] & { comingSoon?: boolean } }) {
  return (
    <div className="w-full max-w-sm bg-white rounded-3xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 h-full flex flex-col justify-between">
      {/* Logo */}
      <div className="mb-4">
        <div className="relative w-12 h-12">
          <Image src={card.logo || "/placeholder.svg"} alt={card.title} fill className="object-contain" priority />
        </div>
      </div>

      {/* Title */}
      <h3 className="text-2xl md:text-3xl font-black text-[#3D1F3F] mb-3">{card.title}</h3>

      {/* Description */}
      <p className="text-gray-700 text-sm md:text-base leading-relaxed mb-6 flex-grow">{card.description}</p>
    ...
    <div className="relative w-24 h-24 md:w-32 md:h-32">
      ...
      <div className="absolute inset-2 md:inset-3 bg-white rounded-full flex items-center justify-center border-4 md:border-6 border-orange-500">
        ...
        <div className="relative w-10 h-10 md:w-14 md:h-14">
          <Image src="/logos/center-logo.jpg" alt="Center Logo" fill className="object-contain" priority />
        </div>
      </div>
      ...
      <div className="hidden md:block absolute bottom-1/2 right-full w-32 lg:w-48 h-0.5 bg-gradient-to-l from-orange-400 to-transparent translate-y-1/2 mr-2"></div>
      ...
      <div className="hidden md:block absolute -top-full left-1/2 h-32 lg:h-48 w-0.5 bg-gradient-to-t from-orange-400 to-transparent -translate-x-1/2 -mb-2"></div>
    </div>
  )
}
